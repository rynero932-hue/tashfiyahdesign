...
async function loadProducts() {
  try {
    let snap;
    try { snap = await db.collection(PRODUCTS_COL).orderBy("order", "asc").get(); }
    catch { snap = await db.collection(PRODUCTS_COL).get(); }
    allProducts = snap.docs.map(d => ({ firestoreId: d.id, ...d.data() }));
  } catch { allProducts = []; }
  renderStats(); renderDashProducts(); renderAdminProducts();
}
...
function renderDashProducts() {
  ...
  if (!allProducts.length) { wrap.innerHTML = "<p style='color:#888;padding:20px 0'>Belum ada produk.</p>"; return; }
...
function renderAdminProducts() {
  ...
  if (!allProducts.length) { wrap.innerHTML = "<p style='color:#888;padding:20px'>Belum ada produk.</p>"; return; }
  wrap.innerHTML = allProducts.map((p, i) => `
    <div class="admin-prod-row">
      ...
      <div class="admin-prod-info">
        <div class="admin-prod-name">${p.name}${p.badge ? ` <span class="prod-badge ${p.badge.toLowerCase()}">${p.badge}</span>` : ""}</div>
        <div class="admin-prod-cat">${p.category}</div>
        <div class="admin-prod-price">${fmt(p.price)}</div>
      </div>
      <div class="admin-prod-actions">
        <button class="btn-edit" onclick="openProductForm(${i})">Edit</button>
...
function openProductForm(idx) {
  ...
  if (editProductIdx !== null) {
    ...
    ["pf-id","pf-name","pf-category","pf-price","pf-oldprice","pf-spec","pf-desc","pf-colors","pf-sizes","pf-images","pf-combinations"]
...
async function saveProduct() {
  ...
  try {
    if (editProductIdx !== null && allProducts[editProductIdx].firestoreId) {
      await db.collection(PRODUCTS_COL).doc(allProducts[editProductIdx].firestoreId).update(product);
      toast("Produk berhasil diperbarui!");
    } else {
      product.createdAt = new Date().toISOString();
      await db.collection(PRODUCTS_COL).add(product);
...
async function deleteProduct(firestoreId, idx) {
  ...
  try {
    await db.collection(PRODUCTS_COL).doc(firestoreId).delete();
...
async function loadPayments() {
  try {
    const snap = await db.collection(PAYMENTS_COL).get();
  ...
  wrap.innerHTML = allPayments.map((p,i) => `
    <div class="payment-row">
      ...
      <div class="payment-row-actions">
        <button class="btn-edit" onclick="openPaymentForm(${i})">Edit</button>
...
async function savePayment() {
  ...
  try {
    if (editPaymentIdx !== null && allPayments[editPaymentIdx].firestoreId) {
      await db.collection(PAYMENTS_COL).doc(allPayments[editPaymentIdx].firestoreId).update(payment);
      toast("Rekening diperbarui!");
    } else {
      await db.collection(PAYMENTS_COL).add(payment);
      toast("Rekening ditambahkan!");
    }
    closePaymentForm(); await loadPayments();
  } catch (err) { toast("Gagal: " + err.message); }
  btn.textContent = "Simpan"; btn.disabled = false;
}
async function deletePayment(firestoreId, idx) {
  ...
  try {
    await db.collection(PAYMENTS_COL).doc(firestoreId).delete();
    toast("Rekening dihapus"); await loadPayments();
  } catch (err) { toast("Gagal: " + err.message); }
}
...
async function saveStore() {
  ...
  try {
    if (newPw) {
      ...
      await db.doc(ADMIN_DOC).set({ password: newPw }, { merge: true });
    }
...
async function seedDefaultData() {
  ...
  const defaultPayments = [
    ...
    { name:"Bank BNI", accountNumber:"0175181167", accountName:"SOLIHAT", icon:"https://upload.wikimedia.org/wikipedia/commons/3/35/BNI_logo.svg", order:2 },
